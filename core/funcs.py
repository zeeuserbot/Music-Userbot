...
async def search(message: Message) -> Optional[Song]:
    ...
    if reply:
        if reply.text:
            ...
            if not media:
                return None
            lel = await message.reply_text("`Mencoba Mengunduh...`")
            file = await reply.download(
                progress=progress_bar,
                progress_args=("Mengunduh...", lel, time.time()),
            )
            await lel.delete()
            return Song(
...
async def progress_bar(current, total, ud_type, msg, start):
    ...
    if round((now - start) % 3) == 0 or current == total:
        ...
        current_message = f"**Mengunduh...** `{round(percentage, 2)}%`\n`{progressbar}`\n**Selesai**: `{humanbytes(current)}` | **Total**: `{humanbytes(total)}`\n**Speed**: `{humanbytes(speed)}/s` | **ETA**: `{time_to_complete}`"
        if msg:
            try:
